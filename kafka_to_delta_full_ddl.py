...
if __name__ == '__main__':
    ...
    KAFKA_TOPIC = "oracle-log-stream-04.MASERA.STREAMTABLE_DDL"
    ...
    newdf = df.select(
        ...
        col('data.after.COL3').alias('COL3'),
        col('data.after.COL4').alias('COL4'),
    ...
    newdf.select('ID','COL1','COL2','COL3','COL4','DATA_REF','INTEGRATED_AT')\
        .writeStream\
        .format("delta")\
        .outputMode("append")\
        .option("checkpointLocation", f"gs://{bucketprefix}-bronze/_offtopics_ctl/{table_name}/")\
        .option("overwriteSchema", "true")\
        .start(f"gs://{bucketprefix}-bronze/{table_name}")\
        .awaitTermination()
