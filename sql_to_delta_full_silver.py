...
if __name__ == '__main__':
    ...
    DeltaTable.createIfNotExists(spark) \
        ...
        .location(f"gs://{bucketprefix}-silver/bigtable_last_month-{actual_year}-{actual_month}") \
        .execute()

    # write data to silver
    df.write.format("delta").mode("overwrite").save(f"gs://{bucketprefix}-silver/bigtable_last_month-{actual_year}-{actual_month}")
